...
```{r setup}
# As this is the first time you are downloading data for project 188, set new = TRUE and update = TRUE.  This will create a .motus file in your current working directory, which was shown above using getwd() This will also create an SQL object in your R environment called 'sql.motus'
tbl(sql.motus,"alltags")%>% #determine which tags have detections by using the count function
  filter(tagProjID == proj.num)%>%
  count(motusTagID)%>%
  as.data.frame()
#filter out run lengths less than 3
...
df.alltagsproj188 contains 253 observations of 62 variables
```{r select tag IDs}
#To select certain tag IDs:
df.alltagsproj188 <- select(tbl.alltags, recv, port, motusTagID) %>% 
  distinct() %>% collect() %>% as.data.frame()
df.alltagsproj188 <- filter(tbl.alltags, motusTagID %in% 
...
write.csv(df.alltags, "./data/Alltagdetections.csv")
...
df.tagDeps <- tbl.tagDeps %>%
                ...
                mutate(tsStart = as_datetime(tsStart, tz = "UTC", origin = "2018-06-01"),
                       tsEnd = as_datetime(tsEnd, tz = "UTC", origin = "2019-08-31")) 
...
write_csv(TvsFdetections,"./data/true_vs_falsedetections.csv")
...
# df.alltags has 73125 obs of 43 variables. After filtering run lengths greater than 3, we created another object called df.alltags.sub that has 23,778 obs of 43 variables
...
fun.getpath <- function(df) 
  ...
  df %>%
    ...
    filter(recvLat< 35) %>% # filter out everything north of 35 degrees latitude# drops data without lon/lat
    group_by(motusTagID, runID, recvDeployName, 
             tagDepLat,tagDepLon, recvLat, recvLon) %>%
...
write_csv(df.alltags.path, "./data/df.alltags_path_RL2_Feb132020.csv")
